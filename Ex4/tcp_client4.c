...
int main(int argc, char **argv)
{
	...
	rt = (len/(float)ti);  
	throughput = 8*rt;                                       //caculate the average transmission rate
	printf("Time(ms) : %.3f, Data sent(byte): %d\nData rate: %f (Kbytes/s)\nThroughput: %f (Kbits/s)\n", ti, (int)len, rt, throughput);

	close(sockfd);
	fclose(fp);

	//logging results
    FILE * logfp = fopen("results.csv", "a");
    char csv_string[50];
    sprintf( csv_string, "%d,%d,%.3f,%ld,%.3f\n", BATCHSIZE, DATALEN, ti, len, throughput);
	...
	while(ci<= lsize)
	{
		...
		if (count % BATCHSIZE == 0)
		{
			...
			if (ack.num != 1|| ack.len != 0)
				...
				printf("ACK received %d\n", count/BATCHSIZE);                     // keep track of number of ACK recieved
				ci += slen;
			}
		}
		else{
